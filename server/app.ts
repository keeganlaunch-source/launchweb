...
// Analytics hub moved to specific route (no longer at root)

app.use((req, res, next) => {
  if (req.path === '/analytics-hub.html') {
    try {
  ...
  if (req.path === '/launch-hub-direct.html') {
    try {
      ...
      console.error('Direct hub error:', error);
      res.status(404).send('File not found');
      return;
    }
  }
...
// Direct download route for iPad
...
// URGENT: Direct route before any middleware
app.get('/get-hub', (req, res) => {
...
// Direct email endpoint - bypasses any middleware issues
app.use(express.json());
app.use(express.urlencoded({ extended: false }));

app.post('/api/direct-email-test', async (req, res) => {
  try {
    ...
    if (emailSent) {
      res.json({ success: true, message: "Consistency hacks email sent successfully" });
    } else {
...
let ready: Promise<{ app: express.Express; server: import("http").Server }> | null = null;

/**
 * Builds the fully-wired Express app (API routes, cron endpoints, error handler)
 * without starting a listener or attaching Vite. Used by both the long-running
